...
while iterations < 11:
    ...
    #u_prev
    si.set_parameters(9,0,u_0[0])
    si.set_parameters(9,1,u_0[1])

    A = np.array([[1.0,0.0,-u_0[0]*np.sin(x_0[2])], [0.0,1.0,u_0[0]*np.cos(x_0[2])], [0.0,0.0,1.0]])
    for i in range(3):
        for j in range(3):
            si.set_parameters(3,i+3*j,float(A[i,j]))

    B = np.array([[h*np.cos(x_0[2]),-u_0[0]*np.sin(x_0[2])*h**2/2], [h*np.sin(x_0[2]), u_0[0]*np.cos(x_0[2])*h**2/2], [0,h]])
    for i in range(3):
        for j in range(2):
            si.set_parameters(4,i+3*j,float(B[i,j]))

    #ref
    si.set_parameters(6,0,float(7))
    si.set_parameters(6,1,float(3))
    si.set_parameters(6,2,float(0))

    si.set_parameters(7,0,float(7))
    si.set_parameters(7,1,float(3))
    ...
    plt.subplot(2,1,1)
    pred = np.array(si.get_prediction_horizon())
    plt.plot(x1[1:6],linestyle='-', color='r', linewidth=3)
    plt.plot(x2[1:6],linestyle='--', color='b', linewidth=3)
    plt.plot(x3[1:6],linestyle=':', color='g', linewidth=3)
    plt.legend(('x1','x2','x3'))
...
plt.figure(1)
plt.subplot(2,1,1)
plt.plot(xpos[0:11],linestyle='-', color='r', linewidth=3)
plt.plot(ypos[0:11],linestyle='--', color='b', linewidth=3)
plt.plot(thetapos[1:11],linestyle=':', color='g', linewidth=3)
plt.legend(('x1','x2','x3'))

plt.subplot(2,1,2)
plt.plot(u1used[0:11],linestyle='-', color='r', linewidth=3)
plt.plot(u2used[0:11],linestyle='--', color='b', linewidth=3)
plt.legend(('u1','u2'))
plt.show()
